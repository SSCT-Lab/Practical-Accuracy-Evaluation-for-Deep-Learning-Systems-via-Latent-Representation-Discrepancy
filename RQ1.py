...
def print_correlation(eg, nbc, snac, nac, tknc, gentle, acc, lsc):
    print("-------------------------")
    print("EG:", eg)
    print("NBC_gen:", nbc)
    print("SNAC_gen:", snac)
    ...
    print("TKNC_gen:", tknc)
    print("LSC_gen:", lsc)
    print("Gentle_gen:", gentle)
    print("ACC_gen:", ACC_gen)
    print("-------------------------")
    print("Spearman Relation between EG and Gentle:", spearmanr(eg, gentle))
    ...
    print("-------------------------")
    print("Spearman Relation between ACC and Gentle:", spearmanr(acc, gentle))
...
def correlation(batch_size, dataset, dataroot, pre_trained_model, net_type, outf, gpu, trans, aug_transform):
    ...
    if dataset == 'mnist' or dataset == 'fashion':
        train_num = 60000
    EG_gen, NBC_gen, SNAC_gen, NAC_gen, TKNC_gen, Gentle_gen, ACC_gen, LSC_gen = [], [], [], [], [], [], [], []
    ...
    for model_name in model_files:
        ...
        NBC = (nbc_train_and_aug - nbc_train) / (nbc_train_and_ood - nbc_train)
        SNAC = (snac_train_and_aug - snac_train) / (snac_train_and_ood - snac_train)
        if nac_train_and_ood - nac_train == 0:
            NAC = "NAN"
        else:
            NAC = (nac_train_and_aug - nac_train) / (nac_train_and_ood - nac_train)
        if tknc_train_and_ood - tknc_train == 0:
            TKNC = "NAN"
        else:
            TKNC = (tknc_train_and_aug - tknc_train) / (tknc_train_and_ood - tknc_train)
        if lsc_train_and_ood - lsc_train == 0:
            LSC = "NAN"
        else:
            LSC = (lsc_train_and_aug - lsc_train) / (lsc_train_and_ood - lsc_train)
        ...
        Gentle = metrics.calc_gentle(model, train_loader, dataset, batch_size, trans, dataroot, aug_train_loader,
        ...
        Gentle_gen.append(Gentle)
        LSC_gen.append(LSC)
        ACC_gen.append(ACC)

    return EG_gen, NBC_gen, SNAC_gen, NAC_gen, TKNC_gen, Gentle_gen, ACC_gen, LSC_gen
...
if __name__ == '__main__':
    ...
    EG_tot, NBC_tot, SNAC_tot, NAC_tot, TKNC_tot, Gentle_tot, ACC_tot, LSC_tot = [], [], [], [], [], [], [], []
    pre_trained_path = args.pre_trained_model + '/' + args.net_type + '_' + args.dataset + '/'
    for i in range(3):
        EG, NBC_gen, SNAC_gen, NAC_gen, TKNC_gen, Gentle_gen, ACC_gen, LSC_gen = correlation(args.batch_size, args.dataset,
                                                                           args.dataroot, pre_trained_path,
                                                                           args.net_type, args.outf, args.gpu, trans,
                                                                           aug_transform)
        print("------------ times: ", i, " ------------")
        print_correlation(EG, NBC_gen, SNAC_gen, NAC_gen, TKNC_gen, Gentle_gen, ACC_gen, LSC_gen)
        EG_tot.extend(EG)
        NBC_tot.extend(NBC_gen)
        SNAC_tot.extend(SNAC_gen)
        NAC_tot.extend(NAC_gen)
        TKNC_tot.extend(TKNC_gen)
        Gentle_tot.extend(Gentle_gen)
        ACC_tot.extend(ACC_gen)
        LSC_tot.extend(LSC_gen)

    print("----------- final results --------------")
    print("Total number:", len(EG_tot))
    print_correlation(EG_tot, NBC_tot, SNAC_tot, NAC_tot, TKNC_tot, Gentle_tot, ACC_tot, LSC_tot)
